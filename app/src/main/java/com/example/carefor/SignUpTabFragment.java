...
//                    password.requestFocus();
                        ...
                        if (radio.equalsIgnoreCase("Caregiver")) {
                                                                ...
                                                                firebaseAuth.getCurrentUser().sendEmailVerification().addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                    ...
                                                                    public void onComplete(@NonNull Task<Void> task) {
                                                                        if (task.isSuccessful()) {
                                                                            ...
                                                                            databaseReference.child("Caregiver").child(usern).child("Status").setValue("Approval Pending");
...
//                            });
                        ...
                        } else if (radio.equalsIgnoreCase("Client")) {
                            InputMethodManager imm = (InputMethodManager) getActivity().getSystemService(Context.INPUT_METHOD_SERVICE);
                            imm.hideSoftInputFromWindow(view.getWindowToken(), 0);
//                        final String encryptpass = md5(pass);

                            if (!(getYear >= 2018)) {
                                Toast.makeText(getActivity(), "Client has be of Age 4 or above to use CareFor", Toast.LENGTH_SHORT).show();
                            } else {
                                                                ...
                                                                firebaseAuth.getCurrentUser().sendEmailVerification().addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                    ...
                                                                    public void onComplete(@NonNull Task<Void> task) {
                                                                        if (task.isSuccessful()) {
                                                                            ...
                                                                            //sending data to realtime database
                                                                            //we are using email address as unique identity of every user
                                                                            databaseReference.child("Client").child(usern).child("Full_Name").setValue(name);
                                                                            databaseReference.child("Client").child(usern).child("Email_Address").setValue(emailaddress);
                                                                            databaseReference.child("Client").child(usern).child("User_Name").setValue(usern);
                                                                            databaseReference.child("Client").child(usern).child("Phone_Number").setValue(phone_number);
                                                                            databaseReference.child("Client").child(usern).child("Date_OF_Birth").setValue(dateofBirth);
                                                                            databaseReference.child("Client").child(usern).child("Gender").setValue(savegender);
                                                                            databaseReference.child("Client").child(usern).child("Status").setValue("Approval Pending");
//                                                                             databaseReference.child("Client").child(usern).child("password").setValue(encryptpass);
                                                                            Toast.makeText(getActivity(), "User Successfully Registered. Please Verify Email", Toast.LENGTH_SHORT).show();
                                                                            Intent i = new Intent(getActivity(), MainActivity.class);
                                                                            startActivity(i);
...
//                            });
                            ...
                            progressBar2.setProgress(0);
                            progressBar2.setVisibility(View.INVISIBLE);
                            Toast.makeText(getActivity(), "Registration Failed, Try Again Later!", Toast.LENGTH_SHORT).show();
                        }
